...
export default function ResponsesPage() {
  ...
  return (
    <AdminLayout currentPage="responses">
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Sales Rep Assessments</h1>
          <p className="text-slate-600 mt-1">
            {loading ? 'Loading...' : `${responses.length} submission${responses.length !== 1 ? 's' : ''}`}
          </p>
        </div>
        ...
        <div className="bg-blue-50 rounded-xl p-4 border border-blue-200 flex items-center justify-between">
          <div>
            <span className="text-blue-700 font-medium">Survey Link:</span>
        ...
        <div className="space-y-4">
          {responses.map((r) => (
            <div key={r.id} className="bg-white rounded-xl shadow-sm border border-slate-200">
              {/* Summary Row */}
              <div 
                className="p-5 cursor-pointer hover:bg-slate-50 flex items-center justify-between transition-colors"
                onClick={() => toggleExpand(r.id)}
              >
                <div className="flex items-center gap-6">
                  <div>
                    <div className="font-semibold text-lg text-slate-800">{r.respondent_name}</div>
                    <div className="text-slate-500 text-sm">{r.respondent_email}</div>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                ...
                <div className="border-t border-slate-200 p-6 bg-slate-50">
                  <div className="grid gap-6">
                    {TOOLS.map((tool) => {
                      ...
                      return (
                        <div key={tool.key} className="bg-white rounded-lg p-4 border border-slate-200">
          ...
          {!loading && responses.length === 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
              <div className="text-slate-500 text-lg">No responses yet</div>
